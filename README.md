...
The actual point of running this at all. Every open date across everything ever
...
Fully rebuilt from `.inbox-state.json` every run, never hand-edited or appended
to — past dates just drop off. If a scheduled run is set up and you've got a
`signal` MCP connected, the same digest gets delivered as a Signal note-to-self
too, so it actually reaches your phone instead of sitting in a folder. See
[`templates/DEADLINES.md.example`](templates/DEADLINES.md.example) and
...
- [`references/triage.md`](references/triage.md) — scan/classify/propose loop, correction memory, scheduled propose-mode, deadline ledger, digest delivery, partial-run handling
